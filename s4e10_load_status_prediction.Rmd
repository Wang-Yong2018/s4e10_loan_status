...
best_glm_params <- tune_glm_results %>%
  select_best("roc_auc") #  选择 AUC 最高的参数组合
...
tune_lgbm_grid <- 
  ...
  update(trees=trees(c(100L,500L)),
...
#  Option 1: Prediction as a Class
predictions_class <- predict(final_lgbm_model, new_data = your_test_data, type = "class")

#  Option 2: Prediction as a Probability
predictions_prob <- predict(final_lgbm_model, new_data = your_test_data, type = "prob")

#  Combine Results on the Test Data (optional)
lgbm_results <- your_test_data %>%
  bind_cols(predictions_class)  %>% #  For Classes
  bind_cols(predictions_prob)     #  For Probabilities

#  Optionally save the submission file in a format for uploading

write_csv(lgbm_results, file = "lgbm_predictions.csv")
```
