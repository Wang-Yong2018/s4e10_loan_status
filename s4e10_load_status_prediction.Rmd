...
simple_glm_probs %>% roc_auc(loan_status, .pred_0,event_level = 'first)
...
lgbm_fit <- 
  ...
  combined_train_result <-
    bind_cols(
    train_data,
    predict(lgbm_fit,new_data = train_data),
    predict(lgbm_fit,new_data = train_data,type='prob'))
  
  combined_train_result |>
    select(id, loan_status,.pred_class,.pred_0,.pred_1)|>
  yardstick::roc_auc(loan_status, .pred_0,event_level = 'first')
