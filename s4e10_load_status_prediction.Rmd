...
system('kaggle competitions submit -c playground-series-s4e10 -f submission.csv -m "lgbm tune local score 0.95"')
Sys.sleep(5)
# get latest score 
...
# 定义基模型
model_spec_glm <- logistic_reg() %>% set_engine("glm")%>% set_mode("classification")
...
# 训练基模型
base_models <- 
  workflow_set(preproc = list(rcp_bs_rose),  # 使用相同的预处理流程
